...
document.addEventListener('DOMContentLoaded', () => {
    ...
    websocket.onmessage = function(event) {
      try {
        ...
        if (data.type === 'game_defeat') {
          ...
          console.log('💥 Défaite reçue via WebSocket (joueur1):', data);
          stopGlobalTimer();
          showDefeatPopup();
          return;
        }
  ...
  function showDefeatPopup() {
    console.log('🎭 Affichage popup de défaite (joueur1)');
    const popup = document.getElementById('defeatPopup');
    if (!popup) {
      console.error('❌ Popup de défaite non trouvée dans le DOM (joueur1)');
      return;
    }

    popup.style.display = 'flex';
    console.log('✅ Popup de défaite affichée (joueur1)');
  }
